...
#Prices
AXP.z <- get.hist.quote(instrument="AXP", start="2009-01-01", quote="AdjClose", 
                        provider="yahoo", end = "2019-01-01", compression="month")
index(AXP.z) <- as.yearmon(index(AXP.z))

colnames(AXP.z) <- "AXP"
AXP.z <- aggregate(AXP.z, index(AXP.z), tail, 1) # to manage duplicate index
index(AXP.z) <- as.yearmon(index(AXP.z))

# CC Returns
returns <- diff( log(AXP.z[,1]) )
plot(returns,xlab="Dates",      ylab="CC Return", type="l",    main="CC Return for AXP Stock")

#Seasonal Decomposition
fitRet <- stl(returns[,1], s.window="period")
plot(fitRet, main=paste("Seasonal Decomposition of AXP CC Returns Time Series"))
...
plot(new_data, xlab="Year", ylab="CC Return", main="Predictions of CC Return AXP using ARIMA model")
...
arima.forecast_axp <- forecast(data, h = 24,level = c(95,80))
