...
#Prices
IBM.z <- get.hist.quote(instrument="IBM", start="2009-01-01", quote="AdjClose", 
                       provider="yahoo", end = "2019-01-01", compression="month")
index(IBM.z) <- as.yearmon(index(IBM.z))

colnames(IBM.z) <- "IBM"
IBM.z <- aggregate(IBM.z, index(IBM.z), tail, 1) # to manage duplicate index
index(IBM.z) <- as.yearmon(index(IBM.z))

# CC Returns
returns <- diff( log(IBM.z[,1]) )
plot(returns,xlab="Dates", 
     ylab="CC Return", type="l",
     main="CC Return for IBM Stock")

#Seasonal Decomposition
fitRet <- stl(returns[,1], s.window="period")
plot(fitRet, main=paste("Seasonal Decomposition of IBM CC Returns Time Series"))
...
plot(new_data, xlab="Year", ylab="CC Return", main="Predictions of CC Return IBM using ARIMA model")
...
arima.forecast_ibm <- forecast(data, h = 24,level = c(95,80))
plot(arima.forecast_ibm, main = "ARMA forecasts for IBM returns")
